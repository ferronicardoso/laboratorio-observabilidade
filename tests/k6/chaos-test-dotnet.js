...
export const options = {
  stages: [
    ...
    { duration: '5s', target: 5000 },    // CAOS: 5000 VUs em 5s!
    { duration: '20s', target: 5000 },   // Mantém caos
    { duration: '5s', target: 0 },       // Crash rápido
  ],
  thresholds: {
    // Thresholds muito permissivos - esperamos MUITAS falhas
...
export default function () {
  ...
  const endpoints = [
    `${BASE_URL}/api/products?page=${Math.floor(Math.random() * 100)}&pageSize=50`,
    `${BASE_URL}/api/products/${Math.floor(Math.random() * 1000)}`,
